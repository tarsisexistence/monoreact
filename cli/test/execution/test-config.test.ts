...
describe('[bin.execution.test.config]', () => {
  ...
  it('should test in src __tests__ dir', () => {
    const output = smartExec('monoreact test --config customJestSrcTests.config.js');
    expect(output.code).toBe(0);
  });

  it('should fail with no found tests in src level dir', () => {
    const output = smartExec('monoreact test --config customJestSrc.config.js');
    expect(output.code).toBe(1);
  });

  it('should finish positive tests in test dir', () => {
    const output = smartExec('monoreact test --config customJestTest.config.js');
    expect(output.code).toBe(0);
  });

  it('should fail with no found tests in __tests__ dir', () => {
    const output = smartExec('monoreact test --config customJestTests.config.js');
    expect(output.code).toBe(0);
  });
});
